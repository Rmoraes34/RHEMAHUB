...
function AuthPage() {
  ...
  async function handleGoogle() {
    ...
    if (result.error) {
      setBusy(false);
      return toast.error("Falha no login com Google: " + result.error.message);
    }
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="relative hidden flex-col justify-between p-12 text-primary-foreground lg:flex" style={{ background: "var(--gradient-primary)" }}>
        <div className="flex items-center gap-2 text-xl font-bold">
          <Sparkles className="h-6 w-6" /> Rhema Estratégia
        </div>
        <div>
          <h1 className="text-4xl font-extrabold leading-tight">Gestão completa da sua agência</h1>
          <p className="mt-4 max-w-md text-primary-foreground/80">
            CRM, financeiro, contratos, atendimento e organização de clientes — tudo centralizado
            em uma plataforma modular e preparada para o futuro.
          </p>
        </div>
        <p className="text-sm text-primary-foreground/60">© {new Date().getFullYear()} Rhema Estratégia</p>
      </div>

      <div className="flex items-center justify-center bg-background p-6">
        <Card className="w-full max-w-md p-8 shadow-[var(--shadow-soft)]">
          <div className="mb-6 flex items-center gap-2 text-lg font-bold text-primary lg:hidden">
            <Sparkles className="h-5 w-5" /> Rhema Estratégia
          </div>
            ...
            <Tabs defaultValue="login">
              ...
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  <path fill="#EA4335" d="M12 4.75c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 1.4 14.97.42 12 .42A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 6.68 9.14 4.75 12 4.75z" />
                </svg>
              ...
              <TabsContent value="signup">
                <form onSubmit={handleSignup} className="space-y-4 pt-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    O primeiro cadastro se torna administrador. Novos cadastros ficam aguardando
                    aprovação de um administrador antes de acessar a plataforma.
                  </p>
